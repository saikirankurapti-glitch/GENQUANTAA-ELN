...
class NotebookService:
    ...
    async def create_entry(
        self, db: AsyncSession, *, obj_in: NotebookEntryCreate, tenant_id: UUID, current_user: User
    ) -> NotebookEntry:
        ...
        existing = await notebook_repo.get_by_number(
            db, experiment_id=obj_in.experiment_id, entry_number=obj_in.entry_number, tenant_id=tenant_id
        )
        if existing:
        ...
        entry = await notebook_repo.create_entry(
    ...
    async def get_entry(
        self, db: AsyncSession, *, entry_id: UUID, tenant_id: UUID, include_details: bool = True
    ) -> NotebookEntry:
        """Fetch notebook entry by ID or raise NotebookEntryNotFound."""
        entry = await notebook_repo.get_by_id(
            db, id=entry_id, tenant_id=tenant_id, include_details=include_details
        )
        if not entry:
    ...
    async def update_entry(
        ...
        *,
        ...
        entry = await self.get_entry(db, entry_id=entry_id, tenant_id=tenant_id)
        if entry.is_locked:
            raise NotebookEntryLockedError("Cannot update a locked notebook entry.")

        updated_entry, new_version = await notebook_repo.update_entry(
            db, db_obj=entry, obj_in=obj_in, current_user_id=current_user.id
        )
    ...
    async def list_versions(
        self, db: AsyncSession, *, entry_id: UUID, tenant_id: UUID
    ) -> List[NotebookEntryVersion]:
        """Fetch all historical version snapshots for a notebook entry."""
        await self.get_entry(db, entry_id=entry_id, tenant_id=tenant_id, include_details=False)
        return await notebook_repo.list_version_history(db, notebook_entry_id=entry_id)
    ...
    async def add_attachment(
        ...
        *,
        ...
        return await notebook_repo.add_attachment(
            db,
    ...
    async def add_comment(
        ...
        *,
        ...
        await self.get_entry(db, entry_id=entry_id, tenant_id=tenant_id)
        return await notebook_repo.add_comment(
            db,
            notebook_entry_id=entry_id,
    ...
    async def add_tag(
        ...
        *,
        ...
        await self.get_entry(db, entry_id=entry_id, tenant_id=tenant_id)
        return await notebook_repo.add_tag(
            db, notebook_entry_id=entry_id, tag_name=obj_in.tag_name, color=obj_in.color
        )
    ...
    async def list_entries(
        ...
        *,
        ...
        return await notebook_repo.list_entries(
