...
class SampleService:
    ...
    async def create_sample(
        self, *, obj_in: SampleCreate, tenant_id: UUID, current_user: User
    ) -> Sample:
        ...
        existing_code = await sample_repo.get_by_code(experiment_id=obj_in.experiment_id, sample_code=obj_in.sample_code, tenant_id=tenant_id
        )
        if existing_code:
        ...
        existing_barcode = await sample_repo.get_by_barcode(barcode=obj_in.barcode, tenant_id=tenant_id)
        if existing_barcode:
        ...
        sample = await sample_repo.create(obj_in=obj_in, tenant_id=tenant_id, current_user_id=current_user.id
        )
        logger.info(f"SampleService: Registered sample '{sample.sample_code}' with barcode '{sample.barcode}' (ID: {sample.id})")
        return sample

    async def get_sample(
        self, *, sample_id: UUID, tenant_id: UUID, include_details: bool = True
    ) -> Sample:
        """Fetch sample by ID or raise SampleNotFound."""
        sample = await sample_repo.get_by_id(id=sample_id, tenant_id=tenant_id, include_details=include_details
        )
        if not sample:
    ...
    async def get_by_barcode(
        ...
        sample = await sample_repo.get_by_barcode(barcode=barcode, tenant_id=tenant_id)
        if not sample:
    ...
    async def update_sample(
        ...
        *, sample_id: UUID,
        ...
        sample = await self.get_sample(sample_id=sample_id, tenant_id=tenant_id)
        ...
        updated_sample = await sample_repo.update(db_obj=sample, obj_in=obj_in, current_user_id=current_user.id
        )
        logger.info(f"SampleService: Updated sample {sample_id}")
        return updated_sample

    async def archive_sample(
        self, *, sample_id: UUID, tenant_id: UUID, current_user: User
    ) -> Sample:
        """Archive a sample."""
        sample = await self.get_sample(sample_id=sample_id, tenant_id=tenant_id)
        if sample.is_archived:
        ...
        archived = await sample_repo.archive(sample_id=sample_id, tenant_id=tenant_id, current_user_id=current_user.id
        )
    ...
    async def restore_sample(
        ...
        restored = await sample_repo.restore(sample_id=sample_id, tenant_id=tenant_id, current_user_id=current_user.id
        )
    ...
    async def delete_sample(
        ...
        success = await sample_repo.soft_delete(id=sample_id, tenant_id=tenant_id, current_user_id=current_user.id
        )
        if not success:
            raise SampleNotFound(f"Sample {sample_id} not found.")
    ...
    async def list_samples(
        ...
        *, tenant_id: UUID,
        ...
        return await sample_repo.list_samples(tenant_id=tenant_id, filter_params=filter_params, pagination=pagination
        )

    async def get_chain_of_custody(
        self, *, sample_id: UUID, tenant_id: UUID
    ) -> List[SampleChainOfCustody]:
        """Fetch chain of custody history for a sample."""
        await self.get_sample(sample_id=sample_id, tenant_id=tenant_id, include_details=False)
        return await sample_repo.get_chain_of_custody_history(sample_id=sample_id)
