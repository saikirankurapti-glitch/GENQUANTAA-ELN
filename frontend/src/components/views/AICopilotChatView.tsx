...
import { Bot, Sparkles, Send, FileText, CheckCircle2, ArrowUpRight, ShieldAlert, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto h-[calc(100vh-5rem)] flex flex-col">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-5 rounded-2xl text-white shadow-lg flex items-center justify-between border border-slate-800 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Section 2.4 MVP Scope • Grounded in 42 lab notebook records with source citations</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 shrink-0">
        {suggestedPrompts.map((p, idx) => (
          <button
            ...
            className="text-xs bg-white hover:bg-blue-50 text-slate-700 hover:text-blue-600 border border-slate-200 hover:border-blue-300 px-3 py-1.5 rounded-lg whitespace-nowrap transition-colors shadow-sm font-medium"
          >
            💡 {p}
          </button>
        ))}
                ...
                <div className="pt-2 border-t border-slate-200/60 flex items-center gap-2 flex-wrap text-[11px]">
                  ...
                  {msg.citations.map((c, i) => (
                    <button
                      ...
                      className="bg-blue-100 hover:bg-blue-200 text-blue-800 font-mono px-2 py-0.5 rounded border border-blue-300 flex items-center gap-1 transition-colors"
                    >
                      <span>{c.label}</span>
              ...
              {msg.protocolData && (
                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-3 text-slate-800">
                  ...
                  <div className="flex gap-2 pt-2 border-t border-slate-100">
                    ...
                    <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">
                      Copy Text
                    </button>
                  </div>
                </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3 shrink-0">
        ...
        <button
          onClick={() => handleSend()}
          className="bg-teal-600 hover:bg-teal-700 text-white p-2.5 rounded-lg shadow-sm transition-colors"
        >
